	...
	class MenuAL implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (b.getText().equals("외박 신청")) {
				int result = JOptionPane.showConfirmDialog(null, "외박을 신청하시겠습니까?", "경고", JOptionPane.YES_NO_OPTION); // stab.viewP(new
																													// ONRequest());
				if (result == JOptionPane.OK_OPTION) {
					...
					dao.OnRequest();
					JOptionPane.showOptionDialog(null, "외박신청이 완료되었습니다.", "확인", JOptionPane.DEFAULT_OPTION,
			...
			if (b.getText().equals("특이사항 등록")) {
				JOptionPane.showOptionDialog(null, "특이사항 등록 화면으로 이동합니다.", "확인", JOptionPane.DEFAULT_OPTION,
						JOptionPane.INFORMATION_MESSAGE, null, option, option[0]);

				stab.viewP(new DPRequest());
			}
			if (b.getText().equals("로그아웃")) {
				JOptionPane.showOptionDialog(null, "로그아웃합니다.", "확인", JOptionPane.DEFAULT_OPTION,
						JOptionPane.INFORMATION_MESSAGE, null, option, option[0]);
				// dao.disconnect();
				stab.logout();
			}
