...
public class DormManagementDAO {
    ...
    Login login;
    Data data;
        String nameValue, sidValue,deptValue,rnValue,genderValue,pnValue,enValue,milValue,chValue,rlgnValue,addrValue,rDateValue,transValue,tempValue,rtrnValue,pDateValue,subValue;
    String solveValue,state,temp,trans, name, sid, dept, roomNumb, pNumb, eNumb, addr, gender, mil, chis, rlgn, pic, pDate, com, ddate, dest, rdate, rtrn, sub, rsn, date;
    int pnt, sum;
    ...
    void ONReturn(){
        try{
            String sql = "update OnTBL set rDate='"+rDateValue+"', trans='"+transValue+"' , temp='"+tempValue+"' , ret ='"+rtrnValue+"'  where s_name='"+nameValue+"'  and s_id = '"+sidValue+"'";
    ...
    void DPSolve(){
        try{
            String sql = "update DPTbl set state='해결'  where s_name='"+nameValue+"'  and s_id = '"+sidValue+"' and com_sub='"+subValue+"' and P_date ='"+ pDateValue+"' ";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
        }catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    void MngAdd(){
        try{
            String sql = "\n" +
                    "insert into studentTbl values ('"+nameValue+"', '"+sidValue+"', '"+deptValue+"','"+genderValue+"', '"+milValue+"', '"+pnValue+"','"+addrValue+"','"+rnValue+"', '"+enValue+"','"+chValue+"','"+rlgnValue+"','"+pic+"',0);";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
        }catch (SQLException throwables){
            throwables.printStackTrace();
    ...
    void MngEdit(){
        try{
            String sql = "update studenttbl set s_name='"+nameValue+"' , s_id ='"+sidValue+"', dept='"+deptValue+"', gender='"+genderValue+"', mil='"+milValue+"', P_number='"+pnValue+"', addr='"+addrValue+"', r_Number='"+rnValue+"', e_Number='"+enValue+"', C_History='"+chValue+"', Religion='"+rlgnValue+"', pic='"+pic+"' where s_name='"+name+"'and s_id = '"+sid+"' ";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
        }catch (SQLException throwables){
            throwables.printStackTrace();
    ...
    void RnPUpdate(){
        try{
            String sql = "update studenttbl set pnt='"+sum+"' where s_name='"+nameValue+"'  and s_id = '"+sidValue+"'";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
    ...
    void RnPAdd(){
        try{
            String sql = "insert into RnplogTbL " +
                    "values ('0','"+nameValue+"','"+sidValue+"' ,"+pnt+",'"+rsn+"','"+date+"')";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
    ...
    void DPRequest(){
        try{
            String sql = "insert into DpTbL values ('0','"+nameValue+"','"+sidValue+"' ,'"+deptValue+"','"+rnValue+"','"+pnValue+"','"+pDateValue+"','"+com+"','"+subValue+"','진행중')";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
    ...
    void OnRequest(){
        try{
            String sql = "insert into ONTbL values ('0','"+nameValue+"','"+sidValue+"' ,'"+dept+"','"+roomNumb+"','"+pNumb+"','"+ddate+"','','','','','X')";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.executeUpdate(sql);
        }catch (SQLException e){
            e.printStackTrace();
